import piotr from 'images/review_1.png';
import konradSenior from 'images/review_2.png';
import paweł from 'images/review_3.png';
import natalia from 'images/review_4.png';
import anna from 'images/svg/default-female.svg';
import ignacy from 'images/svg/default-male.svg';
import kinga from 'images/svg/default-f.svg';
import konrad from 'images/svg/default-m.svg';

const commentsBlocks = [
  {
    img: piotr,
    name: 'Piotr',
    profession: 'IT Specialist',
    text: `"I had the pleasure of being part of Victoria's class - it was a very effective time for me in terms of developing my language skills. Victoria always used interesting techniques that fully engaged the group, she made sure that everyone spoke (it was hard to keep silent ;) ). Each lesson had different cases that changed regularly, allowing you to explore different aspects of language use. I recommend a lesson with Victoria, you will definitely make progress!"`,
  },
  {
    img: konradSenior,
    name: 'Konrad',
    profession: 'Senior Key Account Manager',
    text: `"Dzięki lekcjom, mówienie w języku angielskim przestało być dla mnie stresujące co bardzo pomogło mi zarówno w rozmowach biznesowych jak i prywatnych z obcokrajowcami. Zdecydowanie polecam zarówno zajęcia grupowe jak i lekcje indywidualne."`,
  },
  {
    img: paweł,
    name: 'Paweł',
    profession: 'Agency Sales Manager',
    text: `"W naszym zespole prowadziliśmy cykl kursów językowych. Zajęcia dydaktyczne z języka angielskiego prowadziła Victoria. Zespół bardzo wysoko ocenił te zajęcia, zarówno pod względem merytorycznym, dopasowania do poziomów, dopasowania do kontekstu biznesowego branży oraz wspaniałe podejście i kontaktowość oraz aktywne zaangażowanie. Prowadząca doskonale potrafiła nawiązać pozytywną relacje ze słuchaczami, zainteresować ich i zaangażować. Polecam gorąco współpracę z Victorą."`,
  },
  {
    img: natalia,
    name: 'Natalia',
    profession: 'Entrepreneur',
    text: `"Doskonała prowadzącą, organizowała biznesowy warsztat dla mojego teamu handlowców, wszystko w formie gier dzięki czemu zaangażowanie zespołu i chęć do nauki była na bardzo wysokim poziomie. Serdecznie polecam współpracę, gdyż widać jak dużo serca wkłada Viktoria w przekazywanie wiedzy swoim podopiecznym."`,
  },
  {
    img: anna,
    name: 'Anna',
    profession: 'Real Estate Agent ',
    text: `" I had many English tutors, but Victoria surprised me with the way she taught lessons. It was a good combination of fun and learning. I really recommend Victoria to everyone!"`,
  },
  {
    img: ignacy,
    name: 'Ignacy',
    profession: 'Real Estate Advisor',
    text: `"Zajęcia-warsztaty były ciekawe i niestandardowe. Przygotowane były przez Panią Victorię specjalnie pod najważniejsze zagadnienia związane z moją pracą. Pani Victoria potrafi stworzyć taką atmosferę, że można poczuć się swobodnie i jak najwiecej nauczyć. Bardzo polecam!"`,
  },
  {
    img: kinga,
    name: 'Kinga',
    profession: 'IT Specialist',
    text: `"Lekcje angielskiego z Victorią to prawdziwa przyjemność. Victoria jest zawsze przygotowana do zajęć, ma ze sobą dodatkowe, ciekawe materiały, na zajęciach panuje przyjazna atmosfera. Victoria jest zawsze uśmiechnięta i pozytywnie nastawiona do studentów. Stara się przygotować materiały w dopasowaniu do potrzeb, a nawet zainteresowań studentów. Jeżeli chcecie nauczyć się angielskiego lub nie stracić kontaktu z językiem to bardzo polecam zajęcia z Victorią."`,
  },
  {
    img: konrad,
    name: 'Konrad',
    profession: 'Key Account Manager',
    text: `"Dzięki lekcjom, mówienie w języku angielskim przestało być dla mnie stresujące co bardzo pomogło mi zarówno w rozmowach biznesowych jak i prywatnych z obcokrajowcami. Zdecydowanie polecam zarówno zajęcia grupowe jak i lekcje indywidualne."`,
  },
];

export default commentsBlocks;
